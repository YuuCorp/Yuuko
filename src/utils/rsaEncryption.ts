 ...
 */
export async function rsaEncryption(item: string, encrypt?: boolean): Promise<string> {
  ...
  if (encrypt) {
    ...
    const cryptoKey = await subtle.importKey('spki', publicRSAData, { name: 'RSA-OAEP', hash: 'SHA-256' }, false, ['encrypt']) as RSAkey;
    ...
    const encryptItem = await subtle.encrypt('RSA-OAEP', cryptoKey, itemBuffer);
  ...
  const cryptoKey = await subtle.importKey('pkcs8', privateRSAData, { name: 'RSA-OAEP', hash: 'SHA-256' }, false, ['decrypt']) as RSAkey;
  ...
  const decryptItem = await subtle.decrypt('RSA-OAEP', cryptoKey, itemBuffer);
