...
export async function syncAnilistUsers(data: SyncUsers) {
    ...
    logger.log("verbose", "Preparing to sync users", { type: "generic", total });
    ...
    for (let i = 0; i < total; i++) {
        ...
        try {
            ...
            if (remaining <= 2) {
                logger.log("warn", "Rate limit nearly exhausted, waiting 60s", { type: "generic", remaining });
...
async function deleteUser(aniListId: number) {
    logger.log("warn", "User has invalid token, deleting from DB", { type: "generic", aniListId });
    await db
