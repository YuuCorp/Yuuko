...
export default {
  ...
  run: async ({ interaction, client }): Promise<void> => {
    ...
    if (type != 'ANIME' && type != 'MANGA')
      return void interaction.reply({ embeds: [EmbedError(`Please specify either manga, or anime as your content type. (Yours was "${type}")`, null, false)] })

    const excludeIDs: number[] = []

    // ^ First we query the user to find what ID-s we should exclude from the search pool.
    GraphQLRequest("GetMediaCollection", vars)
      .then((response) => {
        ...
        if (data && data.lists && data.lists.length > 0) {
          ...
          for (const MediaList of data.lists.filter(MediaList => MediaList!.name != 'Planning')) {
            if (MediaList && MediaList.entries)
          ...
          GraphQLRequest("Recommendations", recommendationVars)
            .then((response) => {
