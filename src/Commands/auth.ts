...
import { EmbedError, Footer, GraphQLRequest, RSACryption, getOptions, updateBotStats } from "../Utils";
    ...
    run: async ({ interaction, client }): Promise<void> => {
        ...
        if (user) {
            try {
                ...
                await db
                    .update(anilistUser)
                    .set({ anilistToken: await RSACryption(token, true), anilistId: data.id })
        ...
        try {
            ...
            await db.insert(anilistUser).values({ discordId: interaction.user.id, anilistToken: await RSACryption(token, true), anilistId: data.id });
            await updateBotStats(client);
            return void interaction.reply({
                embeds: [
                    {
